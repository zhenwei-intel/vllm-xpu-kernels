...
inline T gelu_kernel(const T& x) {
  // x -> 0.5 * x * (1 + erf(x / sqrt(2)))
  // return (T)(0.5f * x * (1.0f + sycl::erf((float)x / sycl::sqrt(2.0f))));
...
void act_kernel(scalar_t* __restrict__ out,          // [..., d]
  ...
  for (int64_t idx = item_ct1.get_local_id(2); idx < d;
    ...
    const scalar_t y = input[token_idx * d + idx];
    out[token_idx * d + idx] = compute<scalar_t, ACT_FN, act_first>(x, y);
  }
}
...
void call_gelu_kernel(torch::Tensor& out, torch::Tensor& input) {
  ...
  queue.submit([&](sycl::handler& cgh) {
    cgh.parallel_for(
        sycl::nd_range<3>(grid * block, block),
        [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
          act_kernel<sycl_t, gelu_kernel, true>(
              (sycl_t*)out_ptr, (sycl_t*)input_ptr, d, item_ct1);
        });
  });
}
                  ...
                  torch::Tensor& input)  // [..., d]
{
  VLLM_DISPATCH_FLOATING_TYPES(
      ...
      [&] { vllm::call_gelu_kernel<scalar_t>(out, input); });
}
